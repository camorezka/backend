...
async def webhook(
    ...
    # ══════════════════════════════════════════════════════════
    # ADMIN PANEL — inline keyboard, только OWNER_ID
    # ══════════════════════════════════════════════════════════

    # ─── Хелпер: главное меню ────────────────────────────────
    async def _send_admin_main(chat_id: int, edit_msg_id: int = None):
        try:
            users_res  = supabase.table("users").select("tg_id", count="exact").execute()
            bets_res   = supabase.table("bets").select("id", count="exact").execute()
            wins_res   = supabase.table("bets").select("id", count="exact").eq("result", "win").execute()
            ...
            new_users  = supabase.table("users").select("tg_id", count="exact").gte("created_at", week_ago).execute()
            new_bets   = supabase.table("bets").select("id", count="exact").gte("created_at", week_ago).execute()
            pending    = (wait_res.count or 0) + (manual_res.count or 0)
            win_rate   = round((wins_res.count or 0) / max(bets_res.count or 1, 1) * 100, 1)
            try:
                ub = await get_userbot()
                ub_status = "✅ онлайн" if ub else "❌ офлайн"
            except Exception:
                ub_status = "❌ офлайн"
        except Exception as e:
        ...
        text = (
            f"<b>⚙️ ADMIN PANEL — Lucky Spin v5</b>\n"
            f"{'─' * 30}\n"
            f"👥 Пользователей:  <b>{users_res.count or 0}</b>  <i>(+{new_users.count or 0} за 7д)</i>\n"
            f"🎰 Ставок:         <b>{bets_res.count or 0}</b>  <i>(+{new_bets.count or 0} за 7д)</i>\n"
            f"🏆 Побед:          <b>{wins_res.count or 0}</b>  <i>({win_rate}% винрейт)</i>\n"
            f"{'─' * 30}\n"
            f"📦 NFT очередь:    <b>{pending}</b> шт.\n"
            f"   ⏳ waiting:     {wait_res.count or 0}\n"
            f"   🔧 manual:      {manual_res.count or 0}\n"
            f"   ✅ done:        {done_res.count or 0}\n"
            f"{'─' * 30}\n"
            f"🤖 Юзербот:        {ub_status}\n"
            f"📬 Gift аккаунт:   {GIFT_ACCOUNT_USERNAME}\n"
            f"{'─' * 30}\n"
        ...
        keyboard = {
            "inline_keyboard": [
                [
                    {"text": "👤 Найти юзера",     "callback_data": "adm:find_user"},
                    {"text": "📋 Все юзеры",        "callback_data": "adm:list_users"},
                ],
                [
                    {"text": "🚫 Забанить",         "callback_data": "adm:ban_menu"},
                    {"text": "✅ Разбанить",         "callback_data": "adm:unban_menu"},
                ],
                [
                    {"text": "⭐ Выдать звёзды",    "callback_data": "adm:give_stars"},
                    {"text": "🎰 Сбросить цикл",    "callback_data": "adm:reset_cycle"},
                ],
                [
                    {"text": "🎁 Бесплат. спин",    "callback_data": "adm:free_spin"},
                    {"text": "📦 NFT очередь",       "callback_data": "adm:pending_nft"},
                ],
                [
                    {"text": "📢 Рассылка",          "callback_data": "adm:broadcast"},
                    {"text": "🔎 Аудит-лог",         "callback_data": "adm:audit_log"},
                ],
                [
                    {"text": "🏆 Топ игроков",       "callback_data": "adm:top_players"},
                    {"text": "💰 Топ рефералов",     "callback_data": "adm:top_refs"},
                ],
                [
                    {"text": "🤖 Статус юзербота",   "callback_data": "adm:userbot_status"},
    ...
    def _back_kb():
        import json as _j
        return _j.dumps({"inline_keyboard": [[{"text": "◀️ Назад", "callback_data": "adm:back"}]]})

    # ─── Обработчик callback_query ───────────────────────────
    callback_query = data.get("callback_query", {})
    if callback_query:
        cb_sender = callback_query.get("from", {}).get("id")
        ...
        # Главное меню / обновление
        if action in ("back", "refresh"):
            await _answer_cb(cb_id, "🔄 Обновлено" if action == "refresh" else "")
            await _send_admin_main(cb_chat, edit_msg_id=cb_msg_id)
            return {"ok": True}

        # ── Список юзеров (последние 10) ──
        if action == "list_users":
            await _answer_cb(cb_id)
            try:
                res = supabase.table("users").select("tg_id,username,first_name,total_cycles,stars_balance,is_banned").order("created_at", desc=True).limit(10).execute()
                lines = []
                for u in (res.data or []):
                    ban = " 🚫" if u.get("is_banned") else ""
                    name = u.get("first_name") or u.get("username") or str(u["tg_id"])
                    lines.append(f"• <code>{u['tg_id']}</code> {name}{ban} | 🏅{u.get('total_cycles',0)} | ⭐{u.get('stars_balance',0)}")
                text = "<b>👥 Последние 10 юзеров:</b>\n\n" + "\n".join(lines)
                ...
                await tg_api("editMessageText", {"chat_id": cb_chat, "message_id": cb_msg_id,
                    "text": f"❌ {e}", "reply_markup": _back_kb()})
            return {"ok": True}

        # ── Найти юзера — запросить ввод ──
        if action == "find_user":
            await _answer_cb(cb_id)
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "🔎 <b>Найти юзера</b>\n\nОтправь:\n<code>/ainfo &lt;tg_id или @username&gt;</code>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}

        # ── Забанить — запросить ввод ──
        if action == "ban_menu":
            await _answer_cb(cb_id)
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "🚫 <b>Забанить юзера</b>\n\nОтправь:\n<code>/aban &lt;tg_id&gt; [причина]</code>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}

        # ── Разбанить — запросить ввод ──
        if action == "unban_menu":
            await _answer_cb(cb_id)
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "✅ <b>Разбанить юзера</b>\n\nОтправь:\n<code>/aunban &lt;tg_id&gt;</code>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}
        ...
        if action == "give_stars":
            ...
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "⭐ <b>Выдать звёзды</b>\n\nОтправь:\n<code>/astars &lt;tg_id&gt; &lt;кол-во&gt; [заметка]</code>\n\nОтрицательное число — снять звёзды.",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}
        ...
        if action == "reset_cycle":
            ...
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "🎰 <b>Сбросить цикл юзера</b>\n\nОтправь:\n<code>/areset &lt;tg_id&gt;</code>\n\n<i>Сбрасывает cycle_spin=0, winning_spin=3</i>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}
        ...
        if action == "free_spin":
            ...
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "🎁 <b>Выдать бесплатный спин</b>\n\nОтправь:\n<code>/afreespin &lt;tg_id&gt;</code>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}
        ...
        if action == "broadcast":
            ...
            await tg_api("editMessageText", {
                "chat_id": cb_chat, "message_id": cb_msg_id,
                "text": "📢 <b>Рассылка всем юзерам</b>\n\nОтправь:\n<code>/abroadcast &lt;текст сообщения&gt;</code>\n\n<i>Поддерживается HTML-разметка</i>",
                "parse_mode": "HTML", "reply_markup": _back_kb(),
            })
            return {"ok": True}

        # ── NFT очередь ──
        if action == "pending_nft":
            await _answer_cb(cb_id)
            try:
                now = datetime.utcnow().isoformat()
                res = supabase.table("inventory").select("id,tg_id,nft_name,nft_stars,status,available_at").in_("status", ["waiting","manual"]).order("available_at").limit(15).execute()
                lines = []
                for r in (res.data or []):
                    at = str(r.get("available_at","?"))[:10]
                    lines.append(f"• #{r['id']} tg:<code>{r['tg_id']}</code> {r.get('nft_name','?')} {r.get('status','')} 📅{at}")
                text = f"<b>📦 NFT в очереди ({len(res.data or [])} шт.):</b>\n\n" + ("\n".join(lines) or "Очередь пуста ✅")
                await tg_api("editMessageText", {
                    "chat_id": cb_chat, "message_id": cb_msg_id,
                    "text": text, "parse_mode": "HTML", "reply_markup": _back_kb(),
                })
            except Exception as e:
                await tg_api("editMessageText", {"chat_id": cb_chat, "message_id": cb_msg_id,
                    "text": f"❌ {e}", "reply_markup": _back_kb()})
        ...
        if action == "audit_log":
            await _answer_cb(cb_id)
            try:
                res = supabase.table("audit_log").select("tg_id,action,created_at").order("created_at", desc=True).limit(12).execute()
                lines = []
                for r in (res.data or []):
                    dt = str(r.get("created_at",""))[:16].replace("T"," ")
                    lines.append(f"<code>{dt}</code> <b>{r.get('action','?')}</b> id:{r.get('tg_id','?')}")
                text = "<b>🔎 Аудит-лог (последние 12):</b>\n\n" + "\n".join(lines)
                await tg_api("editMessageText", {
                    "chat_id": cb_chat, "message_id": cb_msg_id,
                    "text": text, "parse_mode": "HTML", "reply_markup": _back_kb(),
                })
        ...
        if action == "top_players":
            await _answer_cb(cb_id)
            try:
                res = supabase.table("users").select("tg_id,username,first_name,total_cycles,cycle_spin").order("total_cycles", desc=True).limit(10).execute()
                lines = []
                medals = ["🥇","🥈","🥉"] + ["🏅"]*7
                for i, u in enumerate(res.data or []):
                    name = u.get("first_name") or u.get("username") or str(u["tg_id"])
                    spins = (u.get("total_cycles") or 0) * 5 + (u.get("cycle_spin") or 0)
                    lines.append(f"{medals[i]} {name} — {u.get('total_cycles',0)} цикл. / {spins} спин.")
        ...
        if action == "top_refs":
            await _answer_cb(cb_id)
            try:
                res = supabase.table("users").select("tg_id,username,first_name,referral_count,stars_balance").gt("referral_count",0).order("referral_count",desc=True).limit(10).execute()
                lines = []
                for i, u in enumerate(res.data or []):
                    name = u.get("first_name") or u.get("username") or str(u["tg_id"])
                    lines.append(f"{i+1}. {name} — 👥{u.get('referral_count',0)} реф. / ⭐{u.get('stars_balance',0)}")
        ...
        # ── Статус юзербота ──
        if action == "userbot_status":
            await _answer_cb(cb_id)
            try:
                ub = await get_userbot()
                if ub:
                    me = await ub.get_me()
                    ub_info = f"✅ <b>Онлайн</b>\n👤 @{me.username}\n🆔 {me.id}"
                else:
                    ub_info = "❌ <b>Недоступен</b>\n\nПересоздай USERBOT_SESSION на Render."
                text = f"<b>🤖 Юзербот статус</b>\n\n{ub_info}\n\n📬 Gift аккаунт: {GIFT_ACCOUNT_USERNAME} (<code>{GIFT_ACCOUNT_TG_ID}</code>)"
                kb = _json.dumps({"inline_keyboard": [
                    [{"text": "🔄 Переподключить", "callback_data": "adm:ub_reconnect"}],
                    [{"text": "◀️ Назад", "callback_data": "adm:back"}],
                ]})
                await tg_api("editMessageText", {
                    "chat_id": cb_chat, "message_id": cb_msg_id,
        ...
        # ── Переподключить юзербота ──
        if action == "ub_reconnect":
            await _answer_cb(cb_id, "🔄 Переподключаю...")
            global _userbot_client
            _userbot_client = None
            try:
                ub = await get_userbot()
                result = "✅ Переподключено!" if ub else "❌ Не удалось переподключить."
            except Exception as e:
                result = f"❌ {e}"
            await tg_api("answerCallbackQuery", {"callback_query_id": cb_id, "text": result, "show_alert": True})
            return {"ok": True}
        ...
        if action == "settings":
            ...
            try:
                ...
                for s in (res.data or []):
                    lines.append(f"• <b>{s['key']}</b>: <code>{s.get('value','')}</code>")
                text = "<b>⚙️ Текущие настройки:</b>\n\n" + ("\n".join(lines) or "Нет настроек") + "\n\n<i>Изменить: /aset &lt;key&gt; &lt;value&gt;</i>"
                await tg_api("editMessageText", {
                    "chat_id": cb_chat, "message_id": cb_msg_id,
                    "text": text, "parse_mode": "HTML", "reply_markup": _back_kb(),
                })
    ...
    # ─── Все /a* команды только для OWNER_ID ─────────────────
    if msg_text.startswith("/a") and message.get("from", {}).get("id") != OWNER_ID:
        return {"ok": True}

    sender_id = message.get("from", {}).get("id")

    # ─── /ainfo <tg_id или @username> ────────────────────────
    if msg_text.startswith("/ainfo"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split(maxsplit=1)
        if len(parts) < 2:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /ainfo &lt;tg_id или @username&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        q = parts[1].strip()
        try:
            try:
                search_id = int(q)
                res = supabase.table("users").select("*").eq("tg_id", search_id).single().execute()
            except ValueError:
                q_clean = q.lstrip("@")
                res = supabase.table("users").select("*").ilike("username", q_clean).single().execute()
            u = res.data or {}
            if not u:
                await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "❌ Юзер не найден."})
                return {"ok": True}
            bets_r = supabase.table("bets").select("id,status,result,created_at").eq("tg_id", u["tg_id"]).order("created_at",desc=True).limit(5).execute()
            bets_text = ""
            for b in (bets_r.data or []):
                icon = "🏆" if b.get("result") == "win" else "🎰"
                bets_text += f"\n  {icon} #{b['id']} {b['status']} {b['created_at'][:10]}"
            ban_text = f"🚫 ДА ({u.get('ban_reason','')})" if u.get("is_banned") else "✅ Нет"
            text = (
                f"<b>👤 Юзер {u['tg_id']}</b>\n"
                f"Имя: {u.get('first_name','')} @{u.get('username','?')}\n"
                f"{'─'*25}\n"
                f"🏅 Циклов:     <b>{u.get('total_cycles',0)}</b>\n"
                f"🎰 Спин:       <b>{u.get('cycle_spin',0)}/{u.get('winning_spin',3)}</b>\n"
                f"⭐ Звёзды:     <b>{u.get('stars_balance',0)}</b>\n"
                f"👥 Рефералов:  <b>{u.get('referral_count',0)}</b>\n"
                f"🚫 Бан:        {ban_text}\n"
                f"📅 Рег:        {str(u.get('created_at','?'))[:10]}\n"
                f"\n<b>Последние ставки:</b>{bets_text or ' нет'}"
            )
            import json as _json
            kb = _json.dumps({"inline_keyboard": [
                [{"text": f"🚫 Забанить {u['tg_id']}", "callback_data": f"adm:ban_menu"},
                 {"text": f"⭐ Дать звёзды",            "callback_data": f"adm:give_stars"}],
                [{"text": "🎁 Бесплат. спин",           "callback_data": f"adm:free_spin"},
                 {"text": "🔄 Сбросить цикл",           "callback_data": f"adm:reset_cycle"}],
                [{"text": "◀️ Меню",                    "callback_data": "adm:back"}],
            ]})
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": text, "parse_mode": "HTML", "reply_markup": kb})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /aban <tg_id> [причина] ─────────────────────────────
    if msg_text.startswith("/aban") and not msg_text.startswith("/abroadcast"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split(maxsplit=2)
        if len(parts) < 2:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /aban &lt;tg_id&gt; [причина]", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            target_id = int(parts[1])
            reason = parts[2] if len(parts) > 2 else "admin_ban"
            supabase.table("users").update({"is_banned": True, "ban_reason": reason}).eq("tg_id", target_id).execute()
            log_action(OWNER_ID, "admin_ban_cmd", {"target": target_id, "reason": reason})
            await tg_send_message(target_id, "🚫 Твой аккаунт заблокирован. Обратись в поддержку.")
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ <b>{target_id}</b> заблокирован.\nПричина: {reason}", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /aunban <tg_id> ─────────────────────────────────────
    if msg_text.startswith("/aunban"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split()
        if len(parts) < 2:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /aunban &lt;tg_id&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            target_id = int(parts[1])
            supabase.table("users").update({"is_banned": False, "ban_reason": None}).eq("tg_id", target_id).execute()
            log_action(OWNER_ID, "admin_unban_cmd", {"target": target_id})
            await tg_send_message(target_id, "✅ Твой аккаунт разблокирован. Удачи!")
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ <b>{target_id}</b> разблокирован.", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /astars <tg_id> <кол-во> [заметка] ─────────────────
    if msg_text.startswith("/astars"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split(maxsplit=3)
        if len(parts) < 3:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /astars &lt;tg_id&gt; &lt;кол-во&gt; [заметка]", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            target_id = int(parts[1])
            amount = int(parts[2])
            note = parts[3] if len(parts) > 3 else ""
            user_r = supabase.table("users").select("stars_balance").eq("tg_id", target_id).single().execute()
            old = (user_r.data or {}).get("stars_balance") or 0
            new_bal = max(0, old + amount)
            supabase.table("users").update({"stars_balance": new_bal}).eq("tg_id", target_id).execute()
            log_action(OWNER_ID, "admin_adjust_stars_cmd", {"target": target_id, "delta": amount, "old": old, "new": new_bal})
            sign = "+" if amount >= 0 else ""
            await tg_send_message(target_id, f"{'⭐ Тебе начислено' if amount > 0 else '⭐ Списано'} {abs(amount)} звёзд. Баланс: {new_bal}⭐")
            await tg_api("sendMessage", {"chat_id": OWNER_ID,
                "text": f"✅ Юзер <b>{target_id}</b>: {old} → <b>{new_bal}</b> ({sign}{amount}⭐)\n{note}", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /areset <tg_id> ─────────────────────────────────────
    if msg_text.startswith("/areset"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split()
        if len(parts) < 2:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /areset &lt;tg_id&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            target_id = int(parts[1])
            supabase.table("users").update({"cycle_spin": 0, "winning_spin": 3}).eq("tg_id", target_id).execute()
            log_action(OWNER_ID, "admin_reset_cycle_cmd", {"target": target_id})
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ Цикл юзера <b>{target_id}</b> сброшен.", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /afreespin <tg_id> ──────────────────────────────────
    if msg_text.startswith("/afreespin"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split()
        if len(parts) < 2:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /afreespin &lt;tg_id&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            target_id = int(parts[1])
            supabase.table("users").update({"free_spin_at": None}).eq("tg_id", target_id).execute()
            log_action(OWNER_ID, "admin_free_spin_cmd", {"target": target_id})
            await tg_send_message(target_id, "🎁 Тебе выдан бесплатный спин! Заходи в игру.")
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ Бесплатный спин выдан юзеру <b>{target_id}</b>.", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /abroadcast <текст> ─────────────────────────────────
    if msg_text.startswith("/abroadcast"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split(maxsplit=1)
        if len(parts) < 2 or not parts[1].strip():
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /abroadcast &lt;текст&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            broadcast_text = parts[1].strip()
            users_res = supabase.table("users").select("tg_id").execute()
            tg_ids = [u["tg_id"] for u in (users_res.data or []) if u.get("tg_id")]
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"📤 Рассылка для <b>{len(tg_ids)}</b> юзеров...", "parse_mode": "HTML"})
            sent = 0
            failed = 0
            for uid in tg_ids:
                try:
                    await tg_api("sendMessage", {"chat_id": uid, "text": broadcast_text, "parse_mode": "HTML"})
                    sent += 1
                except Exception:
                    failed += 1
            log_action(OWNER_ID, "admin_broadcast_cmd", {"sent": sent, "failed": failed})
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ Готово: отправлено <b>{sent}</b>, ошибок <b>{failed}</b>.", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}

    # ─── /aset <key> <value> ─────────────────────────────────
    if msg_text.startswith("/aset"):
        if sender_id != OWNER_ID: return {"ok": True}
        parts = msg_text.split(maxsplit=2)
        if len(parts) < 3:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": "Укажи: /aset &lt;key&gt; &lt;value&gt;", "parse_mode": "HTML"})
            return {"ok": True}
        try:
            ...
            log_action(OWNER_ID, "admin_set_setting_cmd", {"key": key, "value": value})
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"✅ Настройка сохранена:\n<b>{key}</b> = <code>{value}</code>", "parse_mode": "HTML"})
        except Exception as e:
            await tg_api("sendMessage", {"chat_id": OWNER_ID, "text": f"❌ Ошибка: {e}"})
        return {"ok": True}
